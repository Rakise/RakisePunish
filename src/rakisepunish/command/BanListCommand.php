...
use rakisepunish\util\ArrayPage;
...
class BanListCommand extends Command {
    ...
    public function __construct() {
        parent::__construct("punicoes");
        $this->description = "Lista todas as puniçoes de um jogador do servidor.";
        $this->usageMessage = "/punicoes <name | ip> [page]";
        $this->setPermission("rakisepunish.command.punicoes");
    }
                ...
                if (count($args) >= 2) {
                    ...
                    if (is_numeric($pageString)) {
                        if (intval($pageString) > $arrayPage->getMaxPages() || intval($pageString) <= 0) {
                            $sender->sendMessage(TextFormat::GOLD . "Coloque uma pagina valida.");
                            return false;
                        }
                        $page = intval($pageString);
                    } else {
                        $sender->sendMessage(TextFormat::GOLD . "\"" . $args[1] . "\" não é uma pagina válida.");
                        return false;
                    }
                }
                $sender->sendMessage(TextFormat::DARK_GREEN . "--[" . TextFormat::GREEN . "tem " . strval(count($names)) . " " . (strtolower($args[0]) == "name" ? "players" : "IP address") . " banidos desse servidor." . TextFormat::DARK_GREEN . "]--");
                if (count($names) >= 1) {
                    foreach ($arrayPage->yieldFromPage($page) as $nameValue) {
                        $sender->sendMessage(TextFormat::AQUA . $nameValue);
                    }
                } else {
                    $sender->sendMessage(TextFormat::GOLD . "Não tem nada para mostrar nessa lista.");
                }
                $sender->sendMessage(TextFormat::GREEN . "------------[Pagina (" . strval($page <= $arrayPage->getMaxPages() ? $page : "1") . " / " . strval($arrayPage->getMaxPages()) . ")]------------");
